...
test('registry serves conditional manifests and accepts bounded drift reports', async (t) => {
  ...
  assert.match(landingPageHtml, /Your browser chat\./)
  assert.match(landingPageHtml, /http:\/\/127\.0\.0\.1:8766\/v1/)
  assert.match(landingPageHtml, /tether-browser/)
  assert.match(landingPageHtml, /\/site\.css\?v=20260725-1/)
  assert.match(landingPageHtml, /\/site\.js\?v=20260725-1/)
  assert.match(landingPage.headers.get('content-security-policy'), /script-src 'self'/)

  const stylesheet = await fetch(`${base}/site.css?v=20260725-1`)
  ...
  const script = await fetch(`${base}/site.js?v=20260725-1`)
