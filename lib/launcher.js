...
export async function probeAdapter({ port = DEFAULT_PORT, fetchImpl = fetch } = {}) {
  try {
    ...
    const currentContract = response.ok && body?.status === 'ok' &&
      body?.service === 'tether-adapter' && body?.mode !== 'xpose'
...
export async function runTetherCli(args, options = {}) {
  ...
  if (args[0] === 'launch' && args[1] === 'codex-app') {
    const { runCodexAppLaunch } = await import('./codex-app-launcher.js')
    return runCodexAppLaunch(args.slice(2), options.codexAppOptions)
  }
  if (args[0] === 'launch') {
    throw new Error(`Unknown launch target: ${args[1] ?? '(missing)'}`)
  }
