...
export async function runTetherCli(args, options = {}) {
  ...
  if (args[0] === '--update') {
    if (args.length !== 1) throw new Error('The "tether --update" command does not accept additional arguments.')
    const { updateTether } = await import('./updater.js')
    await updateTether(options.updateOptions)
    return 0
  }
  if (args[0] === 'xpose') {
  ...
  throw new Error(
    ...
    'Use "tether xpose", "tether --reset", "tether --update", or "tether extension-path".',
  )
}
